...
import datetime
import bokeh
import nodes_lists_multiple_databases
import temporalis_functions
...
widget_autocomplete_input_method = pn.widgets.AutocompleteInput(
    ...
    options=list_of_useeio_method0_names1[1],
    case_sensitive=False,
    search_strategy='includes',
    placeholder='Write something her...',
    min_characters = 0
...
widget_amount_activity = pn.widgets.FloatInput(name='Amount of Process', value=None, step=1e-1)

#https://panel.holoviz.org/reference/widgets/ArrayInput.html
widget_amount_cut_off = pn.widgets.FloatInput(name='Cut-off', value=None, step=1e-1)
...
def select_database_method(search_string):
    # https://docs.brightway.dev/en/latest/content/gettingstarted/objects.html#object-selection
    selected_method = [
        str(method) for method in list_of_useeio_method0_names1[0]
        if str(search_string).lower() == str(method).lower()
    ][0] # this just selects the first element in the list
...
def update_interactive_elements_lca(event):
    ...
    selected_method = select_database_method(search_string=widget_autocomplete_input_method.value)
    ...
    nodes_list,edges_list,chosen_lca=nodes_lists_multiple_databases.create_nodes_and_edges_lists(
        chosen_database=useeio, 
        user_activity_name=selected_activity['name'],
        user_location='',
        user_amount=widget_amount_activity.value, 
        user_method=selected_method, 
        user_cutoff=widget_amount_cut_off.value)
    
    if widget_checkbox_market.value : 
        user_market='auto'
    else : 
        user_market=None

    if widget_checkbox_transport : 
        user_transport='auto'
    else : 
        user_transport=None

    adjusted_nodes_list=nodes_lists_multiple_databases.adjust_nodes_list(
        chosen_database=useeio,
        df_nodes=nodes_list,
        df_edges=edges_list,
    ...
    dataframe_for_temporalization=nodes_lists_multiple_databases.create_dataframe_for_temporalisation(
        df_nodes_adjusted=adjusted_nodes_list,
        df_edges=edges_list
    )
    dataframe_for_user=nodes_lists_multiple_databases.create_dataframe_for_user(
        df_for_temporalisation=dataframe_for_temporalization
    )
    df_activities=updating_col_data_frame(dataframe_for_user)
    #df_activities=updating_col_data_frame(mon_df)
    widget_tabulator.value = df_activities
    widget_tabulator.loading = False

    
def update_interactive_elements_temporalization():
    selected_activity = select_database_activity(search_string=widget_autocomplete_input_activity.value)
    selected_method = select_database_method(search_string=widget_autocomplete_input_method.value)
    nodes_list,edges_list,chosen_lca=nodes_lists_multiple_databases.create_nodes_and_edges_lists(
        chosen_database=useeio, 
...
dummy_df = pd.DataFrame(np.array([
    ...
    [datetime.datetime(2023, 7, 1), 0.5, 'c flow', 'another activity'],
    [datetime.datetime(2024, 7, 1), 1, 'c flow', 'third activity'],
    [datetime.datetime(2025, 4, 1), 4, 'c flow', 'fourth activity'],
    [datetime.datetime(2026, 1, 1), 2, 'c flow', 'fourth activity'],
    [datetime.datetime(2028, 1, 1), 0.2, 'd flow', 'sixth activity'],
    [datetime.datetime(2019, 1, 1), 0.7, 'e flow', 'fiths activity'],
]), columns=['date', 'impact', 'flow', 'activity'])
...
# https://panel.holoviz.org/reference/widgets/Button.html#buttonhttps://panel.holoviz.org/reference/widgets/Button.html#button
widget_button_tabulator.on_click(update_bokeh_pane)
